...
# =====================================================================
# 路线 A：动态流式连接 (Pipelined DP Partition) - Parallel 版本
# =====================================================================
def run_2way_join_pipelined_parallel(table_A, table_B, D, eps_total=1.0, delta_total=1e-6, num_threads=10):
    print(f"\n{'='*70}\n 🚀 [路线 A] 启动 2-Way Join (Pipelined DP Partition - Parallel)\n{'='*70}")
    ...
    print(f"\n[Phase 1] 基础表 DP Partition & Padding (使用 {num_threads} 线程并行)...")
    # ✅ 修改点 1：替换为 process_single_table_parallel，并传入 num_threads
    parts_A, bucks_A = process_single_table_parallel(
        "Table_A", table_A.payloads, 'Key', D, 
        eps_stage, delta_stage, eps_stage, delta_stage, meta_A.b, num_threads=num_threads
    )
    
    # ✅ 修改点 2：Table_B 同理
    parts_B, bucks_B = process_single_table_parallel(
        "Table_B", table_B.payloads, 'Key', D, 
        eps_stage, delta_stage, eps_stage, delta_stage, meta_B.b, num_threads=num_threads
    )
...
if __name__ == "__main__":
    ...
    # 3.parallel
    run_2way_join_pipelined_parallel(tA, tB, DOMAIN_SIZE, eps_total=1.5, delta_total=1e-6,num_threads=10)
