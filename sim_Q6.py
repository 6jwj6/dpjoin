...
# =====================================================================
def run_q6_3way_join_pipelined(table_A, table_B, table_C, D, eps_total=1.0, delta_total=1e-6):
    print(f"\n{'='*70}\n [路线 A] Q6 真实数据: 流式连接 (Pipelined)\n{'='*70}")
    ...
    print("\n[Phase 2] 执行 1st Join: Order ⨝ Trans ...")
    meta_AB = meta_A.join(meta_B)
    joiner_AB = DPJoiner(epsilon=eps_stage, delta=delta_stage, sensitivity=meta_AB.b)
    t_j1_s = time.time()
    parts_AB, bucks_AB = joiner_AB.run_full_join(parts_A, bucks_A, parts_B, bucks_B, merge_factor=2, meta_A=meta_A, meta_B=meta_B)
    print_join_summary(parts_AB, bucks_AB, time.time() - t_j1_s)

    print("\n[Phase 3] 执行 2nd Join: (Order ⨝ Trans) ⨝ Disp ...")
    meta_ABC = meta_AB.join(meta_C)
    joiner_ABC = DPJoiner(epsilon=eps_stage, delta=delta_stage, sensitivity=meta_ABC.b)
    t_j2_s = time.time()
    parts_ABC, bucks_ABC = joiner_ABC.run_full_join(parts_AB, bucks_AB, parts_C, bucks_C, merge_factor=3, meta_A=meta_AB, meta_B=meta_C)
...
# =====================================================================
# 路线 B：重切分 (Repartitioned) - 模拟路由打断的物理开销
# =====================================================================
def run_q6_3way_join_repartitioned(table_A, table_B, table_C, D, eps_total=1.0, delta_total=1e-6):
    print(f"\n{'='*70}\n [路线 B] Q6 真实数据: 重切分 (Repartitioned)\n{'='*70}")
    global_start_time = time.time()
    
    ratio_meta = 0.50 
    eps_meta = eps_total * ratio_meta
    delta_meta = delta_total * ratio_meta
    # 多了重新 Partition 和 Bucketing，分成 6 份
    ...
    print("\n[Phase 3] 执行 2nd Join: (Order ⨝ Trans)_new ⨝ Disp ...")
    meta_ABC = meta_AB.join(meta_C)
    joiner_ABC = DPJoiner(epsilon=eps_stage, delta=delta_stage, sensitivity=meta_ABC.b)
    t_j2_s = time.time()
    parts_ABC, bucks_ABC = joiner_ABC.run_full_join(parts_AB_new, bucks_AB_new, parts_C, bucks_C, merge_factor=3, meta_A=meta_AB, meta_B=meta_C)
...
# =====================================================================
# 路线 C：均匀切分 (Uniform) - 考验空桶对倾斜数据的承受力
# =====================================================================
def run_q6_3way_join_uniform(table_A, table_B, table_C, D, binnum, eps_total=1.0, delta_total=1e-6):
    print(f"\n{'='*70}\n [路线 C] Q6 真实数据: 均匀切分 (Uniform) [Binnum={binnum}]\n{'='*70}")
    global_start_time = time.time()
    ...
    joiner_AB = DPJoiner(epsilon=eps_stage, delta=delta_stage, sensitivity=meta_AB.b)
    t_j1_s = time.time()
    parts_AB, bucks_AB = joiner_AB.run_full_join(parts_A, bucks_A, parts_B, bucks_B, merge_factor=1, meta_A=meta_A, meta_B=meta_B)
    print_join_summary(parts_AB, bucks_AB, time.time() - t_j1_s)
    ...
    joiner_ABC = DPJoiner(epsilon=eps_stage, delta=delta_stage, sensitivity=meta_ABC.b)
    t_j2_s = time.time()
    parts_ABC, bucks_ABC = joiner_ABC.run_full_join(parts_AB, bucks_AB, parts_C, bucks_C, merge_factor=1, meta_A=meta_AB, meta_B=meta_C)
...
if __name__ == "__main__":
    ...
    t_order = load_real_table(csv_path=BASE_PATH + 'order.csv', table_name='Order', join_key_name='account_id', filter_attribute='k_symbol', filter_value='LEASING')
    t_trans = load_real_table(csv_path=BASE_PATH + 'trans.csv', table_name='Trans', join_key_name='account_id', filter_attribute='operation', filter_value='VYBER KARTOU')
    t_disp = load_real_table(csv_path=BASE_PATH + 'disp.csv', table_name='Disp', join_key_name='account_id', filter_attribute=None, filter_value=None)
    
    if t_order and t_trans and t_disp and len(t_order.payloads) > 0 and len(t_trans.payloads) > 0 and len(t_disp.payloads) > 0:
        print("\n\n" + "/"*80 + "\n 启动 Q6 全套基准测试 (Benchmark)\n" + "/"*80)
        
        # 路线 A: 流式
        run_q6_3way_join_pipelined(t_order, t_trans, t_disp, D=DOMAIN_SIZE)
        
        # 路线 B: 重切分
        run_q6_3way_join_repartitioned(t_order, t_trans, t_disp, D=DOMAIN_SIZE)
        
        # 路线 C: 均匀网格 (binnum 设置为 100，观察面对真实长尾数据时空桶率的崩溃)
        run_q6_3way_join_uniform(t_order, t_trans, t_disp, D=DOMAIN_SIZE, binnum=8)
    else:
