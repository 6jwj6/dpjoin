...
def run_q5_3way_join_repartitioned(table_A, table_B, table_C, D, eps_total=1.5, delta_total=1e-5):
    ...
    # 初始化 Logger
    logger = ObliviousComplexityLogger(filepath="q5_repartition_complexity.log")
    ...
    t_j1_s = time.time()
    parts_AB, bucks_AB = joiner_AB.run_full_join(
        parts_A, bucks_A, parts_B, bucks_B, meta_A=meta_A,
          meta_B=meta_B, merge_factor=2, logger=logger)
    ...
    t_j2_s = time.time()
    parts_ABC, bucks_ABC = joiner_ABC.run_full_join(
        parts_AB_new, bucks_AB_new, parts_C, bucks_C,
          meta_A=meta_AB, meta_B=meta_C, merge_factor=3,
          logger=logger)
    logger.end_join()
...
# 路线 C：均匀切分 (Uniform)
# =====================================================================
def run_q5_3way_join_uniform(table_A, table_B, table_C, D, binnum, eps_total=1.5, delta_total=1e-5):
    ...
    parts_AB, bucks_AB = joiner_AB.run_full_join(
        parts_A, bucks_A, parts_B, bucks_B, 
        meta_A=meta_A, meta_B=meta_B, merge_factor=1,
        logger=logger)
    logger.end_join()
    print_join_summary(parts_AB, bucks_AB, time.time() - t_j1_s)
    ...
    parts_ABC, bucks_ABC = joiner_ABC.run_full_join(
        parts_AB, bucks_AB, parts_C, bucks_C, 
        meta_A=meta_AB, meta_B=meta_C, merge_factor=1, 
        logger=logger)
    logger.end_join()
